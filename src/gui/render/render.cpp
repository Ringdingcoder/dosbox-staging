...
{
#ifdef WIN32
    // Windows socket errors use WSAGetLastError, not errno
...
static int complete_read(int fd, char *buf, size_t count)
{
    ...
    while (count) {
#ifdef WIN32
        // Windows recv takes int, so clamp to INT_MAX
        int read_size = (count > INT_MAX) ? INT_MAX : (int)count;
        ret = recv(fd, buf, read_size, 0);
#else
...
static int complete_write(int fd, const char *buf, size_t count)
{
    ...
    while (count) {
#ifdef WIN32
        // Windows send takes int, so clamp to INT_MAX
        int write_size = (count > INT_MAX) ? INT_MAX : (int)count;
        ret = send(fd, buf, write_size, 0);
#else
...
uint8_t input_buf[3*MAX_SEND_SIZE], input_buf_lh[3*MAX_SEND_SIZE];
int input_pos, input_pos_lh;
	...
	if (CAPTURE_IsCapturingImage() || CAPTURE_IsCapturingVideo()) {
        ...
        if (shell_ishost && render.src.width==320 && (render.src.height==448 || render.src.height==224)) {
            ...
            if (input_pos >= 2*MAX_SEND_SIZE)
                input_pos = 0;
            if (input_pos_lh >= 2*MAX_SEND_SIZE)
                input_pos_lh = 0;
            ...
            // Get current time using C++ chrono (cross-platform)
            ...
            timebits[0] = total_ns / ns_per_second;  // seconds
            timebits[1] = total_ns % ns_per_second;  // nanoseconds remainder
